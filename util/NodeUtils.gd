...
static func create(
	script: GDScript, parent: Node2D = null, owner: Node2D = null, name: String = ""
):
	var node = Node2D.new()
	node.set_script(script)
	return __create(node, parent, owner, name if name else script.get_path().get_file())
...
static func create_native(
	nativeClass, parent: Node2D = null, owner: Node2D = null, name: String = ""
):
